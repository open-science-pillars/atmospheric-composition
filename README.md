...
A domain capability: discipline Atmospheric Composition inside the Atmosphere sphere. Pillar means sphere: one of the five Earth science spheres; a capability is skills, knowledge signed by its stewards and deterministic checks, delivered as one plugin.
...
Trace gases, aerosols and air quality: column and profile retrievals,
emissions inference, and the chemistry that links them.
...
here relies on: GES DISC (OMI, MLS, OCO-2) and ASDC (TEMPO, CALIPSO). None engaged yet.
